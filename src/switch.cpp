...
QSize Switch::sizeHint() const {
    ...
    auto w = style.indicatorMargin.left() + style.height + style.indicatorMargin.right() +
             fontMetrics().size(Qt::TextSingleLine, text()).width();
...
void Switch::paintEvent(QPaintEvent*) {
    ...
    if(isEnabled()) {
        ...
        p.drawEllipse(thumbRect.center(),
    ...
    if(state == Qt::Checked) {
        ...
        if(!isVisible()) {
            ...
            thumbBrushAnimation->interpolate(
                colorFromOpacity(style.thumbOffBrush, style.thumbOffOpacity), thumbEnd);
            trackBrushAnimation->interpolate(
                colorFromOpacity(style.trackOffBrush, style.trackOffOpacity), trackEnd);
        }
    } else { // Qt::Unchecked
